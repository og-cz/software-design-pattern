...
1. **Only one object** of a class can ever exist.
2. There is **one global way** to access it.

It’s like saying:

> “Create this object only once. If someone asks for it again, give them the same one.”
> 

**Some things in software should only have one instance, like:**
...
**Because if you create many duplicates → things break, become inconsistent, or waste resources.**

### **How Singleton works? (Simple logic)**

The class:

- hides its constructor using `private`
- stores **one** static instance inside the class
- exposes a method `getInstance()` to access that single instance
...
![image.png](/_IMAGES/singleton-pattern-01.png)
...
The above code will work hunky dory as long as the application is 
single threaded. As soon as multiple threads start using the class, 
there's a potential that multiple objects get created. Here's one 
example scenario:
...
- When thread **A** is scheduled again, is when the mischief
begins. The thread was already past the if null condition check and will proceed to new-up another object of `AirforceOne` and assign it to `onlyInstance`. Now there are two different `AirforceOne` objects out in the wild, one with thread A and one with thread B.
...
- Its possible to subclass a singleton class by making the constructor protected instead of private. It might be suitable undersome circumstances. An approach taken in these scenarios is to create a **register of singletons** of the subclasses and the `getInstance` method can take in a parameter or use an environment variable to return the desired singleton. The registry maintains a mapping of string names to singleton objects.