...
**The Builder Pattern is a design pattern used to create complex objects in a step-by-step way.**

**Instead of putting everything inside one long constructor, the Builder pattern separates how an object is built from the final object itself.**

**It provides a dedicated *builder class* that contains methods for setting each part of the object, allowing you to build objects safely, clearly, and with flexible configurations.**

1. **You have a Product** → the final object being created.
2. **You have a Builder** → contains methods to set each part of the product.
3. **You build objects step-by-step** by calling builder methods.
4. **You finish by calling `build()`** to get the final object.

Instead of doing this:

```java
new House(4, 2, true, false, 3, "Modern");
```
...
![image.png](/_IMAGES/builder-pattern-01.png.png)
...
public class Client {
    ...
    public void main() {
        ...
        IAircraft f16 = f16Builder.getResult();
				...
				IAircraft Boeing747 = new IAircraft();
    }
}
...
but can still be thought of as an example of it. Using the `StringBuilder` instance we can successively create a string by using the `append` method.
...
• The above method can appear in a director or client code and different document types can be built by varying the concrete type of the Document Builder passed into the method. We could have a `HtmlDocumentBuilder` and a `PdfDocumentBuilder` derive from the abstract class `DocumentBuilder` 

## Caveats

- The builder pattern might seem similar to the abstract
factory pattern but one difference is that the builder pattern creates
an object step by step whereas the abstract factory pattern returns the
object in one go.